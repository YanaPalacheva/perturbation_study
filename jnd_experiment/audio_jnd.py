...
def run_trial_session(path_prefix, exp_data, exp_config, win):
    for run in range(general_experiment_configs['num_runs']):
        ...
        output_filename = f"JND_{exp_config['task']}_{exp_data['subject']}_{exp_data['cur_date']}_run_{run}.csv"
        ...
        while trial_index < 100 or reversals < 10:  # stop conditions
            # randomization phase
            ...
            test_stimulus = generate_stimulus_path(path_prefix,
                                                   general_experiment_configs['baseline_pitch'] + current_difference)
            # todo: we don't have a baseline here in data,
            #  so how do we calculate which stimuli to present, difference/2?
